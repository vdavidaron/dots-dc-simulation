...
try:
    ...
    print(f"[ERROR] Error: Could not find {ESDL_FILE_PATH}.")
    sys.exit(1)
...
calculation_services = [
    {
        ...
        "service_image_url": f"{LOCAL_REGISTRY}/datacenter-demand-service:latest",
        "nr_of_models": 0
    },
    {
        "esdl_type": "Battery",
        "calc_service_name": "battery_service",
        "service_image_url": f"{LOCAL_REGISTRY}/battery-service:latest",
        "nr_of_models": 0
    },
    {
        "esdl_type": "PowerPlant",
        "calc_service_name": "power_plant_service",
        "service_image_url": f"{LOCAL_REGISTRY}/power-plant-service:latest",
        "nr_of_models": 0
    },
    {
        "esdl_type": "ElectricityNetwork",
        "calc_service_name": "network_solver_service",
        "service_image_url": f"{LOCAL_REGISTRY}/network-balancer-service:latest",
        "nr_of_models": 0
    },
    {
        "esdl_type": "PVInstallation",
        "calc_service_name": "local_renewable_service",
        "service_image_url": f"{LOCAL_REGISTRY}/local-renewable-service:latest",
        "nr_of_models": 0
    },
    {
        "esdl_type": "GasProducer",
        "calc_service_name": "backup_generator_service",
        "service_image_url": f"{LOCAL_REGISTRY}/backup-generator-service:latest",
        "nr_of_models": 0
    }
]
...
payload = {
    "name": "Datacenter BESS Peak Shaving Simulation (Local)",
...
if response.status_code in [200, 201, 202]:
    print("\n[SUCCESS] Simulation successfully submitted!")
